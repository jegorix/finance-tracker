...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    List<Transaction> findByOccurredAtBetweenAndAccountUserId(
    ...
    @EntityGraph(attributePaths = { "budget", "account" })
    @Query("SELECT t FROM Transaction t WHERE t.account.user.id = :userId")
    List<Transaction> findAllTransactionsWithEntityGraph(@Param("userId") Long userId);
    ...
    @Query("""
            ...
            WHERE a.user.id = :userId
              AND COALESCE(LOWER(b.name), '') LIKE LOWER(CONCAT('%', :budgetName, '%'))
              AND LOWER(a.name) LIKE LOWER(CONCAT('%', :accountName, '%'))
              AND t.amount >= :minAmount
              AND t.amount <= :maxAmount
              AND t.occurredAt >= :startDateTime
